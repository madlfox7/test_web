...
SELECT * FROM (
  ...
  SELECT 'Karen Grigoryan','karen.jpg','Vanadzor','☎️ +374 99 112233','Psychologist-Sexologist','Integrative','hy,ru',9,18000,4.7,'Relationships, intimacy, communication',1 UNION ALL
  SELECT 'Narine Hakobyan','narine.jpg','Yerevan','✉️ narine@mind.am','Cognitive-Behavioral Therapist','CBT; ACT','hy,ru,en',8,15000,4.9,'Anxiety, OCD, perfectionism',1 UNION ALL
  SELECT 'Vahan Stepanyan','vahan.jpg','Gyumri','☎️ +374 77 778899','Psychotherapist','Body-oriented; EMDR','hy,ru',13,21000,4.8,'Psychosomatics, trauma, panic attacks',1 UNION ALL
  SELECT 'Hasmik Melkonyan','hasmik.jpg','Yerevan','tg: @hasmik_psy','Child & Family Therapist','Play therapy; Family','hy,ru,en',7,12000,4.8,'Parent-child relations, behavior issues',1
) AS v(name,picture,city,contacts,specialty,methods,languages,experience,price_from,rating,bio,is_active)
WHERE NOT EXISTS (
...
INSERT INTO specialists (name, picture, city, contacts, specialty, methods, languages, experience, price_from, rating, bio, is_active)
SELECT * FROM (
  SELECT 'Anahit Mkrtchyan','anahit.jpg','Goris','☎️ +374 96 111222','Child Psychologist','Play therapy; CBT','hy,ru',8,10000,4.7,'Emotional development, anxiety in children',1 UNION ALL
  SELECT 'Levon Hovsepyan','levon.jpg','Sevan','tg: @levon_psy','Psychotherapist','Integrative; Existential','hy,ru,en',14,23000,4.9,'Meaning, loss, midlife crisis',1 UNION ALL
  SELECT 'Armine Babayan','armine.jpg','Artashat','✉️ armine@balance.am','Clinical Psychologist','Schema therapy','hy,ru',9,16000,4.8,'Anxiety, depression, emotional balance',1 UNION ALL
  SELECT 'Karen Petrosyan','karenp.jpg','Abovyan','☎️ +374 41 889977','Family Counselor','Systemic; EFT','hy,ru',10,15000,4.7,'Couples, family conflicts, communication',1 UNION ALL
  SELECT 'Lusine Sargsyan','lusine.jpg','Ijevan','tg: @lusine_care','Child & Teen Psychologist','Play therapy; Art therapy','hy,ru',5,9500,4.6,'Behavior, emotions, teenage adaptation',1 UNION ALL
  SELECT 'Hrant Danielyan','hrant.jpg','Gyumri','☎️ +374 55 334466','Clinical Psychologist','CBT; ACT','hy,ru,en',11,18000,4.8,'Anxiety, panic, obsessive thinking',1 UNION ALL
  SELECT 'Diana Hakobyan','diana.jpg','Yerevan','✉️ diana@psycenter.am','Psychotherapist','Gestalt; EMDR','hy,ru,en',9,19000,4.9,'Trauma, identity, self-worth',1 UNION ALL
  SELECT 'Gevorg Grigoryan','gevorg.jpg','Kapan','tg: @gevorg_mind','Addiction Therapist','Motivational interviewing','hy,ru',8,14000,4.6,'Substance abuse, dependence, family issues',1 UNION ALL
  SELECT 'Nune Avetisyan','nune.jpg','Dilijan','☎️ +374 93 556677','Counseling Psychologist','Person-centered; CBT','hy,ru,en',6,11000,4.7,'Self-confidence, life transitions',1 UNION ALL
  SELECT 'Artashes Hambardzumyan','artashes.jpg','Yerevan','☎️ +374 77 447788','Organizational Psychologist','Coaching; CBT','hy,en',12,24000,4.9,'Team dynamics, leadership, stress',1
) AS v(name,picture,city,contacts,specialty,methods,languages,experience,price_from,rating,bio,is_active)
WHERE NOT EXISTS (
  SELECT 1 FROM specialists s WHERE s.name = v.name AND s.city = v.city
);
