...
docker compose exec -T db mariadb -upsyaid -ppsyaidpass psyaid < backend/api_sql_init/002_translations.sql
...
Node‑сервер вынесен в `frontend/tools/node-static/` и не используется контейнерами. Это только для локального превью статики.
...
- Не коммит экспорты БД в корень репозитория. Добавлен `.gitignore`, который игнорирует `*.sql`, кроме инициализационных скриптов в `backend/api_sql_init/`.
...
- В docker-compose сейчас нет Node‑сервиса: Nginx раздаёт `frontend/public/`, а `api/*.php` обслуживает PHP‑FPM. Node полезен только для локальной статики и быстрых экспериментов.
